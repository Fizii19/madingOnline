<x-layout title="MadingBoard - Laporan Postingan">
    <x-navbar :active="'post-reports'" :show-search="false" />
    ...
    <main class="flex-grow w-full max-w-[1440px] mx-auto px-container-padding pt-[100px] pb-container-padding flex flex-col gap-stack-gap">
        <header class="mb-gutter">
            <h1 class="text-display font-sans text-primary">Laporan Postingan</h1>
            <p class="text-secondary mt-unit text-body-lg font-sans">Kelola laporan postingan dari pengguna.</p>
        </header>
        ...
        <div class="flex gap-2 mb-gutter">
            <a href="{{ route('admin.post-reports') }}"
            ...
            <a href="{{ route('admin.post-reports', ['status' => 'pending']) }}"
            ...
            <a href="{{ route('admin.post-reports', ['status' => 'approved']) }}"
            ...
            <a href="{{ route('admin.post-reports', ['status' => 'rejected']) }}"
                        ...
                        <div class="flex-1 min-w-0">
                            <div class="flex items-center gap-2 mb-2 flex-wrap">
                                ...
                                <span class="text-label-caps font-label-caps text-on-surface-variant">
                                    {{ \App\Models\PostReport::REASON_LABELS[$report->reason] ?? $report->reason }}
                                </span>
                            ...
                            <p class="text-sm text-secondary mb-2">
                                ...
                                terhadap postingan oleh <span class="font-bold text-primary">{{ $report->post->author->name }}</span>
                            </p>
                            ...
                            <div class="shadow-neu-inset bg-background rounded-lg p-3 mt-2">
                                <p class="text-xs text-secondary mb-1">Postingan yang dilaporkan:</p>
                                <p class="text-sm font-bold text-primary">{{ $report->post->title }}</p>
                                <p class="text-sm text-on-surface mt-1 whitespace-pre-line">{{ \Illuminate\Support\Str::limit($report->post->content, 200) }}</p>
                            </div>
                        </div>

                        @if ($report->status === 'pending')
                            <div class="flex md:flex-col gap-2 shrink-0">
                                <form method="POST" action="{{ route('admin.post-reports.approve', $report) }}"
                                      onsubmit="return confirm('Setujui laporan ini? Postingan akan dihapus permanen.');">
                                    @csrf
                                    <button type="submit"
                                ...
                                <form method="POST" action="{{ route('admin.post-reports.reject', $report) }}"
